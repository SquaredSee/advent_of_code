...
PUZZLE_INPUT = """4347	3350	196	162	233	4932	4419	3485	4509	4287	4433	4033	207	3682	2193	4223
...
def main():
    row_list = PUZZLE_INPUT.split('\n')  # split on newlines to get rows

    checksum = 0
    for r in row_list:
        row = r.split('\t')  # split on tabs to get number strings
        row = list(map(int, row))  # convert row to list of integers

        difference = abs(max(row) - min(row))
        checksum += difference

    print("Part One: {}".format(checksum))
        ...
        for i in range(0, len(row)):
            for j in range(i, len(row)):
                ...
                if row[i] % row[j] == 0:
                    ...
                    checksum += int(row[j] / row[i])

    print("Part Two: {}".format(checksum))
